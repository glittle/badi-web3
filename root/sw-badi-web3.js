...
self.addEventListener('fetch', function (event) {
  ...
  var isSecure = url.slice(0, 5).toLocaleString() === 'https';
  var isVersionFile = url.indexOf('version.json') !== -1

  var cacheThisRequest = isSecure && !isVersionFile;
  ...
  if (cacheThisRequest) {
    ...
    event.respondWith(
      caches.match(event.request).then(function (response) {
        ...
        console.log('not in cache, fetch from server');
        ...
        return fetch(event.request, {
          ...
          for (var i = 0; i < BLACKLIST.length; ++i) {
            ...
            if (b.test(event.request.url)) {
              shouldCache = false;
              console.log('blacklist - skip', event.request.url)
              break;
            }
          }
